...
class ExerciseViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=["get"], url_path="all_exercises")
    def get_all_exercises(self, request: Request):
        queryset = Exercise.objects.all()
        serializer = ExerciseSerializer(instance=queryset, many=True)
        response = {"message": "List of all created Exercises", "data": serializer.data}
    ...
    def get_exercise_by_id(self, request: Request, pk=None):
        ...
        response = {"message": f"Exercise by its id: {pk}", "data": serializer.data}
        return Response(data=response, status=status.HTTP_200_OK)
    ...
    def update_exercise_by_id(self, request: Request, pk=None):
        ...
        serializer = ExerciseSerializer(
            instance=exercise, data=request.data, partial=True
        )
